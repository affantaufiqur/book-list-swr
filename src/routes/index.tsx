		...
		<main className="container mx-auto min-h-screen py-4">
			<section className="flex flex-row justify-between rounded-md bg-[#F1F0FE] p-12">
				<div className="flex flex-col space-y-3">
					<h1 className="text-xl font-semibold tracking-tight text-purple-primary">MUST READ</h1>
					<h1 className="max-w-lg text-[3rem] font-semibold tracking-tight">{data?.data[0]?.title}</h1>
				...
				<div className="relative inline-block">
					...
					/>
					<img src={data?.data[4]?.image_url} alt="book 3" className="h-[340px]" />
				</div>
			</section>

			<section className="py-24">
				<div className="grid grid-cols-6 gap-16">
					{slicedData?.map((book) => <BookBig key={book.id} data={book} />)}
				</div>
			</section>
